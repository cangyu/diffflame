function SolveFlame5(mdot_f, mdot_o, L, N, ChemTbl_DIR, MAX_ITER)
%Iterate together to solve the counter-flow diffusion flame without filtering.
%Same as SolveFlame3.m, but extra output added.
...
%  MAX_ITER: Maximun number of global iteration.
    ...
    fprintf(fout, '%18s\t%18s\t%18s\t%18s\t%18s\t%18s\t%18s\t%18s\t%18s\t%18s\t%18s\n', ...
        'D', 'kai', 'Z', 'dZdn', 'Y_CH4', 'Y_CO', 'Y_H2', 'Y_H2O', 'Y_CO2', 'T', 'Y_NO');
    for j = 1:N
    ...
    fout = fopen(sprintf('%s/mf=%.6e_mo=%.6e_stat.txt', ChemTbl_DIR, mdot_f, mdot_o), 'w');
    fprintf(fout, '%18s\t%18s\t%18s\t%18s\t%18s', 'z', 'rho', 'u', 'V', 'T');
    for k = 1:K
        fprintf(fout, '\t%18s', NAME{k});
    end
    fprintf(fout, '\t%18s\t%18s\t%18s\t%18s', 'C', 'H', 'O', 'N');
    
    for j = 1:N
        fprintf(fout, '\n%18.8e\t%18.8e\t%18e\t%18e\t%18e', z(j), rho(CUR, j), u(CUR, j), V(CUR, j), T(CUR, j));
        for k = 1:K
            fprintf(fout, '\t%18.8e', Y(CUR, k, j));
        end
        set(gas, 'T', T(CUR, j), 'P', P, 'Y', squeeze(Y(CUR, :, j)));
        Y_C = elementalMassFraction(gas, 'C');
        Y_H = elementalMassFraction(gas, 'H');
        Y_O = elementalMassFraction(gas, 'O');
        Y_N = elementalMassFraction(gas, 'N');
        fprintf(fout, '\t%18.8e\t%18e\t%18e\t%18e', Y_C, Y_H, Y_O, Y_N);
    end
    fclose(fout);
...
function [z, u, T, y] = DiffFlameSim(domain_length, p, tin, mdot_f, mdot_o)
    ...
    tol_ss    = [1.0e-7 1.0e-10];        % [rtol atol] for steady-state problem
