...
%  N: Total num of grid points distributed uniformly.
    ...
    while(err > 1e-3 && max(T(PREV, :)) > 500)
        ...
        while(~ok)
            ...
            %Energy source term Filtering
	        [~,posFlame]=max(T(PREV,:));
	        leftposFlame=max(1,posFlame-Flamelayer/2.0/dz);
	        rightposFlame=min(N,posFlame+Flamelayer/2.0/dz);
	        RS=gsf(leftposFlame,rightposFlame,N,RS);
            
            %Compute energy flux due to species flux
            dYdz = zeros(K, N);
            for k = 1:K
        ...
        %Filtering
        for k=1:K
            RR(k,:)=gsf(leftposFlame,rightposFlame,N,RR(k,:));
        end
    ...
    fout = fopen(sprintf('%s/mf=%.6e_mo=%.6e_filtered.txt', ChemTbl_DIR, mdot_f, mdot_o), 'w');
...
%  leftposFlame: the left boundary position of flame layer
%  rightposFlame: the right boundary position of flame layer
%  N: the total points of the domain
%  res: a 1-D vector to be partly filtered
    ...
    for i=1:N
        if (i>=leftposFlame && i<=rightposFlame)
            ftotal=0.0;
            for j=i-50:i+50
                f(j)=1/(theta*(2*pi)^0.5)*exp(-(j-i)^2/(2*theta));
                ftotal=ftotal+f(j);
            end
            restotal=0.0;
            for j=i-50:i+50
