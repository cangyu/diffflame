...
if __name__ == '__main__':
    origin_chem_tbl_path = os.path.join('..', 'data')
    interp_chem_tbl_path = os.path.join('..', 'ChemTbl')

    if not os.path.exists(origin_chem_tbl_path):
    ...
    data = []
    for tf in os.listdir(origin_chem_tbl_path):
        if tf.endswith('_transformed.txt'):
            case_name = tf.split('_transformed.txt')[0]
            rf = case_name + '_raw.txt'
            raw_data_path = os.path.join(origin_chem_tbl_path, rf)
            trans_data_path = os.path.join(origin_chem_tbl_path, tf)
            print('Processing {} ...'.format(case_name))

            d0r = np.loadtxt(raw_data_path, skiprows=1)
            d0t = np.loadtxt(trans_data_path, skiprows=1)
            cur_pnt_num = len(d0r)
            d0 = np.zeros([cur_pnt_num, NumOfChemTblRcrdElem])
            for i in range(cur_pnt_num):
                d0[i, 0] = d0t[i, 7] # D
                d0[i, 1] = d0t[i, 8] # kai
                d0[i, 2] = d0t[i, 5] # Z
                d0[i, 3] = d0t[i, 6] # dZdn
                d0[i, 4] = d0r[i, 18] # Y_CH4
                d0[i, 5] = d0r[i, 19] # Y_CO
                d0[i, 6] = d0r[i, 5] # Y_H2
                d0[i, 7] = d0r[i, 10] # Y_H2O
                d0[i, 8] = d0r[i, 20] # Y_CO2
                d0[i, 9] = d0r[i, 3] # T
                d0[i, 10] = d0r[i, -18] # Y_NO
